# -*- coding: utf-8 -*-
"""
Created on Thu Sep 21 18:50:30 2017
...
###########################################
#Quadratic regression classifier
...
x1_ = x1.transpose()
for x in x0_:
    temp.append([1,x[0],x[1],x[0]**2, (x[0]*x[1]), (x[1])**2])
...
errrate_quadregress_train = nerr / N
print "Quadratic Regression Error for training data: ", errrate_quadregress_train
Ntest0 = 5000
Ntest1 = 5000
...
for x in xtest0:
    temp.append([1,x[0],x[1],x[0]**2, (x[0]*x[1]), (x[1])**2])
xtest0 = np.array(temp)

for x in xtest1:
    temp1.append([1,x[0],x[1],x[0]**2, (x[0]*x[1]), (x[1])**2])
xtest1 = np.array(temp1)
...
errrate_quadregress_test = nerr / (Ntest0 + Ntest1)
print "Quadratic Regression Error for test data",errrate_quadregress_test
...
for x in xpl:
    for y in ypl:
        yhat = np.matmul([1, x, y, x**2, x*y, y**2], Bhat)
        if(yhat[0] > yhat[1]):
            greenpts.append([x,y])
        else:
            redpts.append([x,y])
